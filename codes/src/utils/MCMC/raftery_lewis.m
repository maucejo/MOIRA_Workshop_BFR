function  result = raftery_lewis(runs, q, r, s, affich)
...
% where:  draws = draws from the sampler (= nvar x ndraws matrix)
...
if affich == 1
    disp(' ')
    disp('========================')
    disp('Raftery-Lewis Diagnostic')
    disp('========================')
    disp(' ')
    disp(['- ', num2str(nmin), ' iterations required (assuming independence) to achieve ', num2str(r), ' accuracy with ', num2str(100*s) ' percent probability'])
    disp(' ')
    disp(['- Thinning factor of ', num2str(kthin), ' required to produce a first-order Markov chain'])
    disp(' ')
    disp(['- ', num2str(nburn), ' iterations to be discarded at the beginning of the simulation (burn-in)'])
    disp(' ')
    disp(['- ', num2str(nprec), ' subsequent iterations required'])
    disp(' ')
    disp(['- Thinning factor of ', num2str(kind), ' required to produce an independence chain'])
end