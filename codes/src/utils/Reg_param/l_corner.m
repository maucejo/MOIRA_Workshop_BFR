function [reg_c,rho_c,eta_c] = l_corner(rho,eta,reg_param0,U,s,b,method,M)
...
if (nargin <= 3)
  method = 'none';
  if (nargin==2), reg_param0 = (1:length(rho))'; end
else
...
if (nargin==8)
  index = find(eta < M);
  rho = rho(index); eta = eta(index); reg_param0 = reg_param0(index);
end
...
elseif (strncmp(method,'tsvd',4) || strncmp(method,'tgsv',4) || ...
  ...
  if (kmax < 0)
    ...
    reg_c = reg_param0(rpi); rho_c = rho(rpi); eta_c = eta(rpi);
  end
...
elseif (strncmp(method,'dsvd',4) || strncmp(method,'dgsv',4))
  ...
  % Compute g = - curvature of L-curve.
  g = lcfun(reg_param0,s,beta,xi,1);
