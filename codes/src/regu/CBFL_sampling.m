function [X, tau_n, tau_f, q] = CBFL_sampling(A, b, L, q0, Ns)
...
[X(:, 1), tau_n(1), tau_f(1), q(1)] = OBR_L(A, b, q0, L);
...
for ee = 2:Ns
    ...
    LX = L*X(:, ee - 1);
    q(ee) = draw_q(q(ee - 1), LX, tau_f(ee - 1));
    tau_f(ee) = gamrand(af + M/q(ee), bf + norm(LX, q(ee))^q(ee));
    tau_n(ee) = gamrand(an + N, bn + norm(b - A*X(:, ee - 1))^2);
    X(:, ee) = draw_F(A, b, L, tau_n(ee), tau_f(ee), q(ee), X(:, ee - 1));   
end
end

%%
function X = draw_F(A, b, L, taun, tauf, q, X0)
M = size(A, 2);

[muX, W] = regL_(A, b, q, L, tauf/taun, X0);
     
Sigma_f = (taun*(A'*A) + tauf*L'*W*L)\eye(M);
