...
if (ps==1)
  ...
  for i=1:ll
    if (nargin==5)
      x_lambda(:,i) = V(:,1:p)*(zeta./(s.^2 + lambda(i)));
      rho(i) = lambda(i)*norm(beta./(s.^2 + lambda(i)));
    else
      x_lambda(:,i) = V(:,1:p)*...
        ((zeta + lambda(i)*omega)./(s.^2 + lambda(i)));
      rho(i) = lambda(i)*norm((beta - s.*omega)./(s.^2 + lambda(i)));
    end
...
elseif (m>=n)
  ...
  for i=1:ll
    if (nargin==5)
      xi = zeta./(s(:,1).^2 + lambda(i)*s(:,2).^2);
      x_lambda(:,i) = V(:,1:p)*xi + x0;
      rho(i) = lambda(i)*norm(beta./(gamma2 + lambda(i)));
    else
      xi = (zeta + lambda(i)*(s(:,2).^2).*omega)./...
           (s(:,1).^2 + lambda(i)*s(:,2).^2);
      x_lambda(:,i) = V(:,1:p)*xi + x0;
      rho(i) = lambda(i)*norm((beta - s(:,1).*omega)./...
               (gamma2 + lambda(i)));
    end
    eta(i) = norm(s(:,2).*xi);
  end
